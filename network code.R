...
option_list <- list(
    ...
    make_option(c("-n", "--number"),metavar="int", dest="num",help="The number of most abundant species you want to plot",default=30,type="integer"),
    make_option(c("-c", "--cut"),metavar="float",dest="cut", help="Specify the threshold of correlation coefficent",default=0.4),
    make_option(c("-l", "--layout"),metavar="string",dest="layout", help="Network layout", default='layout_on_sphere'),
    ...
    make_option(c("--auto-annotation"),metavar="logical",dest="annotation", help="annotate edges according the information in last column", default=TRUE),
...
dat <- read.table(as.character(opt$otu),comment.char="",check.names=F,stringsAsFactors=F, header = TRUE, sep = "\t", quote="")
...
if(opt$num<ncol(dat)){
  sel<-head(order(colSums(dat), decreasing = TRUE), opt$num)
  dat<-dat[,sel]
  if(annotation){
    annotation_row<-annotation_row[sel]
  }
...
#svglite(paste(opt$out,"/","Correlation_network.svg",sep = ""), height=10,width=14)
